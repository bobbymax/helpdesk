...
<div class="row">
	...
	<div class="col-lg-12">
		<form action="{{ route('divisions.update', $division->id) }}" method="POST" class="form-horizontal">
			@csrf
			@method('PATCH')

			<div class="card">
				<div class="card-header">
				    <strong>Edit Division: </strong> {{ $division->name }}
				</div>
				<div class="card-body card-block">
			            ...
			            <div class="col-12 col-md-9">
			                <select name="directorate_id" id="directorate_id" class="form-control">
			                    ...
			                    @foreach($directorates as $directorate)
			                    	<option value="{{ $directorate->id }}" {{ $division->directorate_id === $directorate->id ? ' selected' : '' }}>{{ $directorate->name }}</option>
			                    @endforeach
			                </select>
			            </div>
			        </div>
			        ...
			        <div class="row form-group">
			            ...
			            <div class="col-12 col-md-9">
			                <input type="text" id="abv" name="abv" class="form-control" value="{{ $division->abv }}">
			            </div>
			        </div>
				</div>
